...
SP3_LOCK = threading.Lock() # [VERSIÓN 11]: Candado de seguridad multihilo para la caché SP3
...
def interpolate_sp3(sp3_data, sat, t_emision, degree=9):
    ...
    # [VERSIÓN 11]: Bloqueo multihilo para evitar corrupción de caché
    with SP3_LOCK:
    ...
    # [VERSIÓN 11]: Bloqueo multihilo para escritura
    with SP3_LOCK:
...
def seleccionar_efemeride_optima(eph_list, t_target):
    # [VERSIÓN 11]: Exclusión de efemérides caducadas (>2 horas de diferencia)
...
# =====================================================================
# MOTOR PPK HÍBRIDO DETERMINISTA (VERSIÓN 11 MEJORADA)
# =====================================================================
...
def procesar_ekF_lambda(sd_epoca, nav, sp3, kf_estado, tr, mask_angle, snr_mask):
    try:
        ...
        # [VERSIÓN 11]: Separación Ionosférica (+I para Código, -I para Fase)
        base_calcs = {}
        ...
        for s in sat_list:
            ...
            if data['cp_r'] is not None and data['cp_b'] is not None and rc['cp_r'] is not None and rc['cp_b'] is not None:
                ...
                # [VERSIÓN 11]: Ambigüedad calculada usando el modelo de fase libre de error 2I
...
def tab1_homogenizar():
    with STATE_LOCK:
        # [VERSIÓN 11]: Limpieza absoluta del directorio temporal para evitar colapso de Vercel
    ...
    def procesar():
        try:
            ...
            if not os.path.exists(nav_path):
                ...
                # [VERSIÓN 11]: Liberación de espacio post-descompresión
